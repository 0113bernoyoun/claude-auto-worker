...
const project_config_service_1 = require("../../../src/config/project-config.service");
...
describe('ConfigCommand', () => {
    ...
    beforeEach(async () => {
        const moduleRef = await testing_1.Test.createTestingModule({
            providers: [config_command_1.ConfigCommand, project_config_service_1.ProjectConfigService],
        }).compile();
        command = moduleRef.get(config_command_1.ConfigCommand);
        svc = moduleRef.get(project_config_service_1.ProjectConfigService);
    });
    describe('show 명령어', () => {
    ...
    describe('에러 처리', () => {
        it('서비스 에러 발생 시 적절히 처리되어야 함', async () => {
            ...
            const { outputs, restore } = cli_setup_1.cliTestHelpers.captureConsoleOutput();
            try {
                const result = await cli_setup_1.cliTestHelpers.catchErrors(() => command.run(['path'], {}));
...
//# sourceMappingURL=config.command.spec.js.map